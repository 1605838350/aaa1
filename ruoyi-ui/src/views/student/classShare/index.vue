<template>
  <div class="class-share-page">
    ...
    <div class="page-header">
      ...
      <h2>班级共享</h2>
      <div class="file-count">共 {{ fileList.length }} 份作业</div>
    </div>
    ...
    <div v-loading="loading" class="file-list">
      ...
      <div v-for="group in groupedFiles" :key="group.title" class="homework-group">
        <div class="group-header" @click="toggleGroup(group.title)">
          ...
          <span class="group-count">{{ group.count }} 份</span>
        </div>
        <div v-show="expandedGroups[group.title]" class="group-files">
          ...
          >
            ...
            <div class="file-info">
              ...
              <div class="file-meta">
                <span class="meta-tag student">
                  <i class="el-icon-user"></i>
                  {{ file.account ? file.account.toString().slice(-2) : '' }} {{ file.studentName }}
                </span>
                <span class="meta-item">
                  <i class="el-icon-time"></i>
                  {{ formatTime(file.submitTime) }}
                </span>
                <span v-if="file.score !== null && file.score !== undefined" class="meta-item score">
...
<script>
import { listClassShare, previewStudentFile } from "@/api/student/cloudDisk";
import FilePreview from "@/components/FilePreview";
import mediaViewer from "@/mixins/mediaViewer";

export default {
  name: "StudentClassShare",
  ...
  computed: {
    filteredList() {
      ...
      return this.fileList.filter(f =>
        ...
        (f.studentName && f.studentName.toLowerCase().includes(key)) ||
        (f.account && f.account.toLowerCase().includes(key)) ||
...
@media (max-width: 600px) {
  ...
  .file-meta {
    gap: 8px !important;
    .meta-tag, .meta-item { font-size: 12px; }
  }
}
</style>
